    ...
    def test_dashboard_decision_review_analysis_and_cli_wiring(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            try:
                ...
                self.assertIn("<details>", market_catalog)
                self.assertIn("Все категории", market_catalog)
                self.assertIn("сохранить текущий вид", market_catalog)
                self.assertIn("Macro Signals", market_catalog)
                self.assertGreaterEqual(market_catalog.count("Страница 1 из 1"), 2)
                ...
                self.assertIn("politics", market_catalog_saved)
